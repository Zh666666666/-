...
- 康复指标质量门已改为失败关闭：只接受真实 HARDWARE、匹配当前大腿/小腿设备的 GOOD 校准、200ms 内双路同步对、至少 6 对/3 秒连续观察、合理角速度与完整屈伸周期；任一条件失败时 ROM、次数和关注优先级均保持为空，并返回可追溯原因码。实验性强晃动提示按同一设备连续三帧判断，不把单传感器提示升级成临床风险分。
...
- Web 指标公式：`GET /api/sensor-samples` 同步返回 `metrics`；真实来源、匹配 GOOD 校准、200ms 内双路同步、至少 6 对/3 秒、采样连续、动作合理且完成至少一次完整屈伸后才输出活动范围和关注优先级。当前阈值已完成软件测试，尚未完成四元数相对姿态、功能轴校准、量角器对照和真实 ADL/强晃动误报验证。
