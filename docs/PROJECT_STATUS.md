...
- 邮箱注册代码与数据库迁移已完成，本地测试和生产构建通过；生产开关保持关闭，等待 Resend 发送子域验证和 API Key 后部署真实邮件验收。
...
1. **P0 - 邮箱注册生产启用**
   - 在 Resend 验证 `updates.dorianaistudio.cloud`，将其提供的 DKIM/SPF 记录添加到 Cloudflare，并配置 API Key、发件地址和照护邀请码。
   - 部署迁移后用一个真实邮箱完成“收码→注册→自动登录→再次密码登录”验收，确认护士角色不可公开注册。
2. **P0 - 单传感器本地证据闭环真机验收**
