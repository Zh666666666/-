...
### 2026-09-06 软件交付加固（尚未部署，最终验证进行中）

2026-09-08 续接：上一检查点 `089fc58` 的 GitHub CI 全通过，包括独立 PostgreSQL 迁移和 8 项多账号/并发测试（无跳过）。新增授权真实数据库测试、运维 CLI 生命周期测试、跨连接事件通知和隔离恢复演练继续核验。用户确认尚无异地对象存储/告警接收配置，未购买任何服务，因此真实异地备份及通知仍待配置。
...
- 新增 restic 异地加密备份选项、隔离恢复演练、健康/备份年龄监控与 systemd 单元。真实异地存储、告警收件地址和恢复演练尚未验收，不能宣称容灾达标。
...
优先完成 `docs/SOFTWARE_HARDENING_20260906.md` 的软件验证与发布门槛。硬件长测及算法对照按用户要求暂缓，未完成前不扩大正式医疗交付。

1. 使用两个测试护士账号和一个新注册家属账号完成生产业务验收：验证护士 A 归属码绑定、护士 B 不可查看或抢占、护士 A 释放、护士 B 新码接管和全过程审计；不得使用现有正式患者账号做破坏性测试。
2. 完成多账号隔离验收：家属 A 无法读取患者 B、未关联游客看不到患者数据、护士只访问本人负责患者、解除归属后原护士立即失去访问、改密强制重登、设备换绑不串线。
