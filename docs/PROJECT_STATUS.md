...
- 游客邮箱注册改造已在审计分支完成：页面和两条注册 API 不再提交或校验邀请码，邮件服务就绪条件只依赖 Resend API Key 与发件地址；旧服务器环境中即使保留 `REGISTRATION_INVITE_CODE` 也不会再读取。
...
1. 通过 PR #43 的新一轮 Web/Android 门禁后，备份正式数据库并演练 `20260802120000_add_patient_account_scope`，核对重复活动绑定清理结果与家属患者关联，再合并部署。
...
| --- | --- | --- | --- | --- |
| 2026-08-05 | 取消照护邀请码，开放任意有效邮箱注册家属账号；保留验证码、强密码与家属/护士权限隔离，并增加按 IP、邮箱双层限流；同步清理环境变量、部署文档和 Agent 规则 | 代码与本地门禁通过，待 PR #43 远端门禁、数据库备份和正式部署 | 推送 PR，CI 全绿后备份迁移生产库并部署；公网完成真实邮箱收码与注册验收 | 70/70 Web/网关/API 测试；TypeScript、ESLint、48 路由 build、0 漏洞；390×844 浏览器确认无邀请码字段、发信按钮可用、控制台 0 错误 |
