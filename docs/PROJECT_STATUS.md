...
| --- | --- | --- | --- | --- |
| 2026-08-24 | 完成公开注册后的患者闭环：家属自助建档、护士 48 小时一次性授权码、接受/撤销权限、角色边界与完整访问审计；补充资料页双角色管理界面和注册后建档引导 | 功能位于 `codex/patient-onboarding`，软件和隔离迁移演练通过，尚未合并或部署生产；生产现状未被改动 | 创建 PR 并通过 CI；合并前备份生产数据库，设置独立邀请密钥，部署迁移后完成真实双账号授权与越权验收 | 75/75 Web/网关/API 测试；TypeScript、ESLint、Prisma validate、49 路由生产构建通过；全量迁移在隔离 PostgreSQL 临时库从零执行并删除；桌面/移动家属已关联、未关联建档、护士授权界面无横向溢出且控制台 0 error |
