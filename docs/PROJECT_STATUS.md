...
- 已取消家属邮箱注册的照护邀请码：任何有效邮箱完成 6 位验证码验证并设置安全密码后即可创建家属账号；公开注册仍不能创建护士权限，并增加按 IP 与邮箱双层频率限制。新账号不会自动继承已有患者数据，患者访问继续使用显式账号关联。
...
- 已实现公开的家属邮箱注册：6 位验证码、10 分钟过期、60 秒重发、频率/错误次数限制、验证码哈希、PBKDF2 密码哈希、数据库账号与注册后自动登录；无需照护邀请码，护士账号不开放公开注册。
...
- 已修复登录后桌面侧栏被 `.panel-ink { position: relative }` 覆盖固定定位、因而排到页面内容末尾的问题；应用骨架现在明确使用固定定位、左上角贴边和 `100dvh` 高度，所有登录后页面统一生效，移动端仍保留底部导航。
- 已完成登录后工作台的细节升级：桌面侧栏扩展为带工作区说明、安全会话状态和账号信息的 256px 导航；家属端使用“首页、正在测、训练记录、设备连接、预约护理、我的”等易懂名称，移动端收敛为 5 个高频入口；家属、护士和实时页统一使用紧凑控制台头部、关键指标条、分段工作区与克制的临床中性色画布。
- 已修正两项结果展示边界：实时页只有质量门通过后才允许显示“未发现明显异常”，并明确标出 SSE 实时或轮询降级；护士端没有有效患者样本时，平均屈曲、伸直和训练时长显示 `--`，不再把缺失数据呈现为真实的零值。
...
- 游客邮箱注册、系统级代码审计与安全加固已经通过 PR #43 合并到 `main` 并部署生产：注册页面和两条注册 API 不再提交或校验邀请码，邮件服务就绪条件只依赖 Resend API Key 与发件地址；服务器即使保留旧 `REGISTRATION_INVITE_CODE` 也不会再读取。
- 审计修复覆盖 Supabase 角色提升、家属跨患者访问、网关权限过宽、Cookie 写请求 CSRF、禁用账号重置、改密后旧会话、设备换绑竞态、生产模拟来源、Android 长任务队列扫描、请求体上限、错误泄漏、容器 root/CSP 和依赖漏洞。完整报告、发布边界和回滚步骤见 `docs/SYSTEM_AUDIT_2026-08-02.md`。
- 发布门禁：70 项 Web/网关/API 测试、TypeScript、ESLint、Prisma、48 路由生产构建和 0 漏洞依赖审计通过；GitHub Web Build `30995376462` 与 Android verify `30995376463` 均通过。
- 新注册但尚未关联患者的家属账号默认无权读取任何患者数据；生产迁移 `20260802120000_add_patient_account_scope` 已执行，既有 `local-family` 已关联 `prod-patient-1`，其他家属保持未关联，避免游客自动继承患者资料。
- 默认分支与当前交付基线：`main`；生产部署提交为 `ff0e3fec3e0255eb7decfb1b0def4a510d6675f5`，包含 PR #35 以来的实时链路、后台证据处理、双路均衡指标窗口、易懂版实测预览以及 PR #43 的系统审计与公开邮箱注册。
...
- 账号入口已包含邮箱注册、退出、修改密码与忘记密码；本轮将家属注册改为无需邀请码的公开邮箱验证。设备绑定持久化、历史训练/异常回顾和真实电量来源仍需继续实物验收。
...
- 验证状态：70 项 Web/网关/API 测试、TypeScript、`lint`、48 路由生产 `build` 和依赖漏洞门禁通过；当前合并提交对应 GitHub Build `30995376462` 与 Android verify `30995376463` 通过。Android 发布门禁涵盖 JVM 测试、Android Lint、Debug/Release、R8、zipalign 与 v2/v3/v4 签名校验；浏览器使用双传感器格式并发帧完成指标和响应式验收。
- 云端无密钥时：使用 Demo 模式。
...
- 正式服务器：`103.242.13.17` 已部署 Docker Compose 生产栈（PostgreSQL、Next.js、Caddy），当前源码为 GitHub `main` 合并提交 `ff0e3fec3e0255eb7decfb1b0def4a510d6675f5`（PR #43，系统审计、安全加固与公开邮箱注册），`.deployed-commit` 已同步；本地签名角色会话、业务 API 保护、网关 Bearer Token、日志轮转、每日备份、防火墙与仅密钥 SSH 均已验证。`www.dorianaistudio.cloud` 已解析至正式服务器并取得有效 HTTPS 证书。
...
- 公开邮箱注册已在生产启用：`updates.dorianaistudio.cloud` 的 Resend DKIM/SPF 已验证，API Key 与发件地址仅保存在服务器/本机私密环境；游客无需照护邀请码即可获取验证码并创建家属账号，护士权限仍不开放注册。公网注册页与 Resend 受控投递地址发信接口已验收。
- 仓库结构缺口：外层仓库将 `tka-rehab-platform` 记为 gitlink，且缺少 `.gitmodules`，交付时需固定到应用仓库提交。
- 视觉基线：全站已收敛为单一设计系统，设计令牌集中在 `src/app/globals.css`（色阶、圆角、阴影、缓动、字体栈），基础组件 `src/components/ui/*` 与品牌资产 `src/components/brand.tsx` 均由令牌驱动。拉丁字形通过 `next/font` 加载 Inter，中文交给 PingFang SC / 鸿蒙 / 雅黑系统字体，不额外下载中文字体包。表现层及桌面侧栏固定定位修复已部署到正式服务器。
...
- 2026-08-05 生产部署已完成：发布前备份 `/opt/tka-rehab/backups/tka-rehab-20260805T095939Z.dump`（1,362,878 字节，`pg_restore -l` 118 条），旧源码快照位于 `/opt/tka-rehab/releases/source-20260805T100149Z-422c490.tar.gz`；13 项迁移全部生效，应用、数据库和 Caddy 健康，生产验收与公网 HTTPS 检查通过。

## 下一步任务

按优先级从上到下执行：

1. 设计并实现公开注册后的患者关联流程：新家属可创建或由护士授权关联患者，必须保留明确确认、撤销和审计记录，不能按固定患者或邮箱猜测关联。
2. 完成多账号隔离验收：家属 A 无法读取患者 B、未关联游客看不到患者数据、护士只访问授权患者、改密强制重登、设备换绑不串线。
3. 继续按 `docs/V043_FIELD_TEST_REMEDIATION.md` 做真实双传感器长时弱网验收，使 App 到 Web 最新帧 P95 不超过 2 秒且在线队列收敛。
4. 分离数据质量与患者风险，修复正常屈膝的强晃动/高风险误报；核验 BT50 真实电量，读取不到时显示未知；继续完善训练历史和资料页。
...
| --- | --- | --- | --- | --- |
| 2026-08-05 | 取消照护邀请码，开放任意有效邮箱注册家属账号；完成 PR #43 合并、生产数据库备份与患者范围迁移，并部署 `ff0e3fe` | 正式域名已提供无需邀请码的家属注册；新账号默认不关联患者，护士注册仍关闭；生产三容器健康 | 实现带确认与审计的游客账号→患者关联流程，并完成多账号越权与真机注册验收 | 70/70 Web/网关/API 测试；TypeScript、ESLint、48 路由 build、0 漏洞；GitHub Build `30995376462`、Android `30995376463` 通过；备份 1,362,878 字节、118 条目录；13 项迁移生效；生产验证脚本、ready/login/register 和受控 Resend 发信通过 |
...
| 2026-07-26 | 家属端/护士端 13 个页面/组件全量精修：176 处精确替换（卡片材质/标题层级/等宽数字/色彩纪律/深色令牌化/通用组件），严苛清单 7 项全零通过 | 两端视觉与登录页/首页同一水准，功能零改动；子代理服务全会话故障，严审由主进程按同一清单执行 | 部署至正式服务器并公网验收 | `npm test` 55/55；ESLint 0 告警；48 路由构建；三大页面直审 0 低对比 0 溢出；9 项遗留标记源码级全零 |
...
### 2026-08-05 生产部署：公开邮箱注册与系统审计（ff0e3fe）

PR #43 已合并到 GitHub `main`，合并提交
`ff0e3fec3e0255eb7decfb1b0def4a510d6675f5` 已部署到正式服务器。发布前创建数据库
备份 `/opt/tka-rehab/backups/tka-rehab-20260805T095939Z.dump`（1,362,878 字节），
并通过 `pg_restore -l` 校验得到 118 条可恢复目录记录；旧源码快照保存为
`/opt/tka-rehab/releases/source-20260805T100149Z-422c490.tar.gz`。

迁移 `20260802120000_add_patient_account_scope` 已成功执行，数据库共 13 项迁移且
无待执行项。迁移后设备 ID 和患者部位均不存在重复活动绑定，两项局部唯一索引已
生效。现有 `local-family` 明确关联 `prod-patient-1`，其他家属保持未关联，公开注册
不会自动获得已有患者资料。

Next.js、PostgreSQL 与 Caddy 三容器健康，`.deployed-commit` 已写入完整合并提交。
容器内 `deploy/verify-production.mjs` 通过健康、角色隔离、受保护数据、网关授权、
实时页和传感器流检查；公网 `ready`、登录页和注册页返回 200，注册页存在“无需
邀请码”说明且没有照护邀请码字段。`/api/auth/register/send-code` 已用 Resend 受控
投递地址完成一次无邀请码调用，随后删除测试验证码记录；重复请求按冷却规则返回
429，限流生效。

### 2026-07-30 生产部署：登录后工作台 UI 细节升级（422c490）

GitHub `main` 合并提交 `422c490` 已部署到正式服务器。发布前创建数据库备份
`backups/tka-rehab-20260730T162428Z.dump`（约 1.3MB），并通过容器内
`pg_restore -l` 校验得到 118 条可恢复目录记录；上一版源码快照保存为
`/root/pre-deploy-7ee5e14-20260730.tar.gz`（约 452KB）。部署包由该提交的
精确 `git archive` 生成，本机与服务器 SHA256 均为
`134739247E8D1848EE7866DD1FB5A2E04F02662B8CAEBAB5FF1481FAF034FCA6`。

应用容器重建后 12 个数据库迁移均无待执行项，PostgreSQL、Next.js 与 Caddy
三容器健康，`.deployed-commit` 为 `422c490`。容器内
`deploy/verify-production.mjs` 已通过健康、家属/护士角色隔离、受保护数据、
网关鉴权、实时页和传感器事件流检查；公网 `ready` 与登录页返回 200，安全头
保持生效，裸域携带路径和查询参数永久跳转至 `www`。

### 2026-07-28 生产部署：桌面侧栏固定定位（7ee5e14）

PR #42 合并后将正式服务器从 `66b5198` 升级至 `7ee5e14`。部署前生成数据库备份
`/opt/tka-rehab/backups/tka-rehab-20260728T141648Z.dump`（1,362,878 字节），
并通过容器内 `pg_restore -l` 校验得到 118 条目录记录；旧源码快照为
`/root/pre-deploy-7ee5e14.tar.gz`（451 KB）。使用该合并提交的精确
`git archive` 覆盖程序文件，保留服务器私有 `.env.production` 和历史备份。

生产镜像重建后 PostgreSQL、Next.js、Caddy 三容器正常，应用健康检查通过，
12 个数据库迁移均无待处理项。`verify-production.mjs` 已通过健康、角色隔离、
受保护数据、网关鉴权、登录态实时页和传感器流验证；公网 ready/login 为 200，
未登录访问 `/family` 正确 307 到登录页。生产静态资源已确认包含
`data-app-rail` 固定定位规则，`.deployed-commit` = `7ee5e14`。

### 2026-07-27 生产部署：结构级重排（66b5198）

PR #41 合并后部署正式服务器，生产从 `d8d420a` 升至 `66b5198`。
备份 `tka-rehab-20260726T162354Z.dump`（`pg_restore -l` 103 对象），
快照 `/root/pre-deploy-66b5198.tar.gz`（449 KB）。三容器健康，
`verify-production.mjs` 全项通过；公网 home/login 200、未登录 `/family`
正确 307、CSP 在发、新 UI 标记在线。`.deployed-commit` = `66b5198`。

### 2026-07-27 结构级重排：应用骨架 + 双端指挥台

用户确认精修不够，要求登录后区域做结构级改版（功能与数据流不变，交互
与排版语言大改）。本轮三项结构变更：

1. **持久应用骨架**：`RoleNavigation` 从"底部悬浮坞"升级为桌面端
   240px 固定左侧导航栏（panel-ink 材质、品牌锁定、竖排导航、黄铜
   活动指示条、底部退出/切换），移动端保留底部坞。通过
   `body:has([data-app-rail]) main { margin-left }` 让所有登录后页面
   自动让位——一处改动，整个登录后区域换骨架，未触碰任何页面内部。
   角色获取、切换、退出的 fetch 与句柄逐字保留。
2. **家属端指挥台**：巨幅墨绿海报头（约占首屏一半）替换为紧凑指挥
   台——眉题 + 患者名标题行 + 状态胶囊 + 双动作；新增三格关键读数条
   （屈曲角/阶段目标/数据状态，发丝分隔）；工作区四宫格大卡换成
   贴顶分段控件（滚动时 sticky + 毛玻璃）。
3. **护士端指挥台**：同语言——标题行内嵌同步状态点，六个动作钮收纳
   为一行紧凑按钮组，五页签换贴顶分段控件。

连带修复两个真实缺陷：

- **Recharts 溢出**：侧栏异步挂载导致布局位移后，recharts v3 的
  ResizeObserver 在 React 19 开发模式双挂载下失灵，内联 width 残留
  旧测量值（745px 冲出视口 170px）。修复 = 图表容器改绝对定位包装
  （脱离文档流，永不撑爆网格）+ globals.css 防御性覆盖 recharts
  内联宽高（RO 正常时两值相等，零副作用）。护士端 1 处、实时页
  2 处图表同步处理。
- **本机构建环境结论**（记录防复查）：本机 dev 模式字体抓取不走
  HTTP_PROXY（需 unset 失效代理变量才能启动），build 模式恰相反
  （走 HTTP_PROXY 成功）。香港服务器直连 Google 无此问题。

已验证：

- `npm test` 55/55；ESLint 零告警；48 路由生产构建通过。
- 浏览器实测：桌面 1280 侧栏 240px 渲染、内容让位无重叠、活动项
  正确高亮（/family→监测、/sensor-live→实时）、移动坞在桌面隐藏；
  移动 390 侧栏隐藏、底部坞正常、分段控件可横滚；/family /nurse
  /sensor-live 横向溢出全部为 0；图表宽度 = 容器宽度。
- 功能安全：RoleNavigation 重写前后 fetch/handler 完全一致；两端
  头部替换只动 JSX 结构，所有 onClick/Link/RoleSwitchButton 原样。
...
evidence、appointments、devices、guidance、tcm-knowledge、hardware-demo、
profile、training-history、metric-education-dialog、status-notice）逐一精修。
共 176 处修改，全部为可枚举的精确替换，功能零改动（diff 级扫描确认无
onClick / fetch / useState / disabled 变更）。

变更清单（按类别）：

1. 卡片材质统一（99 处）：实色砂描边 `#e5dbc9/#f2ebdf/#d3c5ac` → 发丝
   描边 `var(--hairline)`/`--hairline-strong`；`shadow-sm/lg/xl` → 多层薄
   阴影 `shadow-e1/e2/e3` 阶梯；空状态虚线框升级 hairline-strong。
2. 页头与标题层级（11 处）：预约/指导/中医三页主标题从随手 `text-5xl/6xl`
   收敛到 `display-md` 阶梯（内页大而空 ≠ 大气）；profile 头部从"底边线
   贴白底"升级为发丝卡片；玫瑰粉色系卡片（rose 不在品牌色板）全部换为
   黄铜暖色渐变。
3. 排版细节（13 处）：所有动态数字读数（屈曲角、统计值）加 `tabular`
   等宽数字防跳动；卡片标题统一 `display-md` 语言。
4. 色彩纪律（33 处）：rose 色系 → brass；图标瓦片 `rounded-3xl` →
   `rounded-xl` 统一阶梯；提示块描边加品牌色低透明度。
5. 深色元素令牌化（12 处）：家属端 `darkPanelClass` 与护士端
   `nurseDarkPanelClass` 从裸 `bg-[#12211c]` 升级为 panel-ink+grain 材质
   （与登录页主视觉同一语言）；野生深色 Button/Badge 覆盖类删除、回归
   组件默认变体；护士端模板按钮与选中态 Tab 换 ink 令牌+内高光。
6. 通用组件（8 处）：StatusNotice 三种语气按品牌语义色重做（描边带
   低透明度品牌色）；profile-form 成功提示同步。

严苛核查（原计划由子代理执行，因子代理服务在本会话持续故障——五次调用
均报模型映射错误——改由主进程按同一份刁钻清单逐项执行，全部可复验）：

- 色彩纪律：品牌外色类 0；非令牌 hex 0（含揪出并消灭的 `#e87ba4` 粉色）。
- 圆角阶梯：任意值圆角 0，全部落在 md/lg/xl/2xl/full 五档。
- 阴影阶梯：shadow-sm/lg/xl 残留 0。
- 描边纪律：实色砂描边残留 0。
- 野生深色背景：`bg-[#12211c]`/`bg-[#0b1512]` 残留 0。
- Button 野生 bg 覆盖：0。
- 同类图标瓦片 padding/圆角一致。
...
- `npm test` 55/55；ESLint 零告警；48 路由生产构建通过。
- `/family`、`/nurse`、`/sensor-live` 桌面直审：0 低对比、0 横向溢出、
  0 遗留标记（iframe 批量审计已被本项目自己的 CSP frame-ancestors 阻断，
  这本身验证了 CSP 生效；改用直接导航+源码级 grep 双重校验）。
