...
- 统一患者病历与主管护士归属已在分支 `codex/patient-record-sync` 完成代码实现：新增迁移 `20260824180000_add_patient_medical_record_and_nurse_owner`、共享病历页、护士完整档案入口、5 秒病历校对和 3.5 秒工作台刷新。现有单护士部署的患者会迁移给最早护士资料；正式部署前仍需备份数据库并在生产副本复核回填归属。
- 本轮门禁：Prisma schema、TypeScript、ESLint、49 路由生产构建和 80 项 Web/网关/API 测试通过；桌面 1440px 与移动 390px 病历页无横向溢出，护士端编辑并保存患者姓名的浏览器流程通过。本机 Docker 未启动，因此尚未执行独立 PostgreSQL 迁移演练，不能在完成生产备份前直接部署。
...
- 默认分支与当前交付基线：`main`；生产部署提交为 `a075a68ea32207af03ae079cb318a2e9444ed01b`，包含 PR #35 以来的实时链路、后台证据处理、双路均衡指标窗口、易懂版实测预览、PR #43 的系统审计与公开邮箱注册，以及 PR #45 的患者建档与授权闭环。
...
- 正式服务器：`103.242.13.17` 已部署 Docker Compose 生产栈（PostgreSQL、Next.js、Caddy），当前源码为 GitHub `main` 合并提交 `a075a68ea32207af03ae079cb318a2e9444ed01b`（PR #45，患者建档、授权码、撤销与审计），`.deployed-commit` 已同步；本地签名角色会话、业务 API 保护、网关 Bearer Token、日志轮转、每日备份、防火墙与仅密钥 SSH 均已验证。`www.dorianaistudio.cloud` 已解析至正式服务器并取得有效 HTTPS 证书。
...
1. 使用一个新注册家属账号和护士账号完成生产业务验收：分别验证自助建档，以及邀请、接受、双方撤销和审计记录；不得使用现有正式患者账号做破坏性测试。
2. 完成多账号隔离验收：家属 A 无法读取患者 B、未关联游客看不到患者数据、护士只访问授权患者、已撤销家属立即失去访问、改密强制重登、设备换绑不串线。
