...
- 已创建 iOS BLE 网关工程：SwiftUI 现场操作界面、Core Bluetooth 双设备接入、BLE5 二进制解析、Keychain 加密离线队列、HTTPS 上传与 iPhone 无 USB 自检；macOS CI 构建待本 PR 验证。
...
- iOS 网关：已建立最低 iOS 17 的独立工程，面向 iPhone 15 Pro（iOS 26.5.2）设计；尚未通过 macOS CI、TestFlight 或真实 WT9011DCL-BT50 验证
- 正式服务器：暂不可用，尚未部署生产环境
...
3. **P0 - 编译与验证 iOS BLE 硬件接入助手**
   - 通过 macOS CI 编译 iOS 工程并运行协议与配置测试。
...
| 2026-07-10 | 新建 iOS 网关：Core Bluetooth、WT BLE5 解析、Keychain 加密队列、平台上传、SwiftUI 无 USB 自检与 macOS CI | iPhone 15 Pro 是首台目标测试机；工程与 CI 尚待验证，未宣称真实 BLE 已连接 | 通过 macOS CI 后，在 iPhone 和 WT9011DCL-BT50 完成协议、权限、双设备、归零与补传验收 | `swift test`、`xcodebuild` 待 GitHub macOS runner 执行 |
