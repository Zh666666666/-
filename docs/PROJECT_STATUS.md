...
- 已实现传感器样本上传后自动触发 AI 分析：当样本带屈曲角并生成康复记录时，服务端自动创建分析报告；同一患者 30 秒冷却，避免 10 Hz 上传刷爆；无真实 API Key 时回退本地规则，后台补上 `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` 后即可对真实采集数据分析。
...
- 已完成官方协议与当前 APK 对照审计，并修复关键偏差：读取官方 `Electricity` 电量、保留广播名、区分 BLE 地址与网关 ID、上传明确 `source=HARDWARE`，共享适配器同时兼容 `Ang*`/`Angle*` 键；审计结论写入 `docs/WITMOTION_PROTOCOL_AUDIT.md`。
...
- Android 网关：v0.2 最低 Android 7.0（API 24）、目标 API 35；Debug 包名改为 `cn.tkarehab.gateway.debug` 以便与正式版共存，Release 保持 `cn.tkarehab.gateway`；GitHub Actions 已生成长期证书签名的 v2/v3/v4 Release APK，并以 Android APK 签名库验证 v4 `.idsig`。Android 15 模拟器已完成冷启动和安装自检，但用户实机仍报告启动即退；已加入仅本地保存的未捕获启动异常诊断页，等待实机回传准确堆栈。尚未用项目 Android 网关连接实物，也尚未完成双传感器、校准和上传验证
- 正式服务器：暂不可用，尚未部署生产环境
- 当前产品性质：可演示、可云端开发；演示数据与真实硬件数据已在来源链上隔离；样本上传后可自动生成 AI 建议（无 Key 时本地规则，有 Key 时真实模型）
- 自动 AI：代码路径已通；真实大模型分析依赖后台配置 `OPENAI_API_KEY` 或 `ANTHROPIC_API_KEY`，尚未在生产域名完成端到端实测
...
1. **P0 - Android 真机闭环**
   - 安装含电量/广播名修复的新 APK，确认扫描、连接、上传与自动 AI 报告。
   - 用 1 只 WT9011DCL-BT50 验证官方 SDK 键值、10 Hz 限流和断网补传。
   - 双传感器安装、直腿归零后对比量角器和官方 App。
2. **P0 - 生产环境 AI 与数据落库**
   - 在 `www.dorianaistudio.cloud` 或正式库配置真实数据库与 AI Key。
   - 验证 HARDWARE 样本入库后自动生成分析，且 DEMO 样本不会被误标为真实。
3. **P1 - 协议增强**
   - 增加原始 `0x55 0x61` 通知旁路存证、四元数读取、厂商序列号确认。
   - 前台服务与断连重连，支撑更长临床会话。
4. **P1 - 自动化测试与安全**
   - 补样本上传自动 AI、重复样本、离线补传和关键流程浏览器回归。
   - 上线前收紧 API 鉴权与 Supabase RLS。
5. **P1 - 正式服务器部署与交付文档**
   - HTTPS、迁移、备份、安装/校准/故障处理文档。
...
| 2026-07-12 | 样本上传后自动触发 AI 分析；抽取 `src/lib/ai-analysis.ts`；对照官方协议审计并修复 APK 电量/广播名/HARDWARE 来源与 `Ang*` 兼容；输出 `docs/WITMOTION_PROTOCOL_AUDIT.md` | 代码与本地网关/Lint/构建验证通过；真实 API Key 与真机端到端尚未跑通 | 安装新 APK 做真机采集上传，后台补 AI Key 后验证自动分析 | `npm run gateway:test`、`npm run check:status`、`npm run lint`、`npm run build` |
