...
- 正式服务器：`103.242.13.17` 已部署 Docker Compose 生产栈（PostgreSQL、Next.js、Caddy），当前源码为 GitHub `main` 合并提交 `422c490`（含全站视觉改版、登录后 256px 应用导航、双端控制台细节升级、实时传输状态与结果展示边界修正），`.deployed-commit` 已同步；本地签名角色会话、业务 API 保护、网关 Bearer Token、日志轮转、每日备份、防火墙与仅密钥 SSH 均已验证。`www.dorianaistudio.cloud` 已解析至正式服务器并取得有效 HTTPS 证书。
...
- 2026-07-30 UI 细节升级已完成本地与生产门禁：55 项运行时/网关/API 测试、ESLint 和 48 路由生产构建通过；浏览器覆盖 9 个登录后入口的 1280px 桌面与 390px 手机视口，桌面侧栏均固定在 `(0,0)`、宽 256px、高度等于视口，全部页面横向溢出为 0；正式服务器已运行 `422c490`，三容器健康，生产验收与公网 HTTPS 检查通过。
...
5. 分离数据质量与患者风险，修复正常屈膝的强晃动/高风险误报；核验 BT50 真实电量，读取不到时显示未知；继续完善训练历史和资料页。
