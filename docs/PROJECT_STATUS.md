...
- Android 网关：v0.2 最低 Android 7.0（API 24）、目标 API 35；Debug 包名 `cn.tkarehab.gateway.debug`。真机已确认可启动、安装自检、扫描并连接单只实物。阻塞点已从“启动/扫描失败”推进到“双传感器校准、实时样本读取、平台上传与生产部署”。
- 正式服务器：暂不可用，尚未部署生产环境
...
1. **P0 - 单传感器实时样本与归零验收**
   - 连接后确认 SDK 输出键（Ang/Acc/As）持续刷新，并与官方 App 对照。
   - 验证“大腿/小腿归零”命令与安装轴向。
2. **P0 - 双传感器膝关节联调**
