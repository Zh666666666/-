...
- 默认分支：`main`；当前注册开发分支：`codex/email-registration`。
...
- 邮箱注册已在生产启用：`updates.dorianaistudio.cloud` 的 Resend DKIM/SPF 已验证，API Key、发件地址和随机照护邀请码仅保存在服务器/本机私密环境；官方测试收件地址已通过生产注册 API 接收请求，测试验证码记录随后清除。尚待用户用真实邮箱完成收码、注册、自动登录和再次密码登录验收。
...
1. **P0 - 邮箱注册真实收件验收**
   - 使用一个尚未注册的真实邮箱完成“收码→注册→自动登录→退出→再次密码登录”，确认邮件未进入垃圾箱、验证码 10 分钟过期、照护邀请码门禁生效且护士角色不可公开注册。
2. **P0 - 单传感器本地证据闭环真机验收**
