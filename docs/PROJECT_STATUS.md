...
- 公开注册后的患者建档与授权关联已在 `codex/patient-onboarding` 完成但尚未部署：未关联家属可明确确认后创建自己的患者档案，或使用护士生成的 48 小时一次性授权码关联既有患者；家属和护士均可撤销访问，建档、邀请、接受与撤销均写入审计记录。授权码只在生成时显示明文，数据库仅保存 HMAC 摘要；数据库迁移已在隔离临时库完整演练，既有账号关联会回填迁移审计。
...
1. 合并并部署患者建档与授权关联：发布前备份数据库，配置独立 `PATIENT_INVITE_SECRET`，执行 `20260824143000_add_patient_onboarding`，再用真实家属/护士账号验收创建、邀请、接受、撤销与审计记录。
...
| --- | --- | --- | --- | --- |
| 2026-08-24 | 完成公开注册后的患者闭环：家属自助建档、护士 48 小时一次性授权码、接受/撤销权限、角色边界与完整访问审计；补充资料页双角色管理界面和注册后建档引导；修复 CI 新发现的 3 项间接依赖安全公告 | 功能位于 `codex/patient-onboarding`，软件和隔离迁移演练通过，尚未合并或部署生产；生产现状未被改动 | 创建 PR 并通过 CI；合并前备份生产数据库，设置独立邀请密钥，部署迁移后完成真实双账号授权与越权验收 | 75/75 Web/网关/API 测试；TypeScript、ESLint、Prisma validate、49 路由生产构建和 `npm audit` 0 漏洞；全量迁移在隔离 PostgreSQL 临时库从零执行并删除；桌面/移动家属已关联、未关联建档、护士授权界面无横向溢出且控制台 0 error |
