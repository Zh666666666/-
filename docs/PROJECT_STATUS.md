...
### 2026-09-09 业务闭环验收进行中（不部署）

- 扩展真实隔离数据库验证：邮箱验证码注册、创建个人档案、主管护士关联、护理指导双方读取、家属已读、预约确认、异常处理及训练历史隔离。邮件外部发送边界使用测试替身，不声称真实邮箱投递通过。
- 修复护理指导已读接口允许护士代确认的问题；重复确认保留首次已读时间。护理记录和预约处理署名改为服务端当前护士资料，不信任请求中的任意署名。
- 完整 CI 和浏览器交互继续核验，尚未发布；线上 App 凭据和训练保持不变。
- 代码 `6f52f14` 的 CI `34325422662` 已通过新增注册、指导、预约、异常处理及历史隔离数据库测试。家属指导页随后补齐加载失败与已读提交失败提示、重试和提交中状态，移除正式轮询中的 Demo 字样。
- `scripts/test-care-ui.py` 在 390px/1280px 双浏览器角色上下文均通过：发送指导、确认阅读、刷新持久化、错误注入与恢复，无未捕获页面异常。浏览器使用本地 demo，生产权限由真实隔离数据库另行验证，不能混同为线上端到端验收。
- 验证说明见 `docs/CARE_WORKFLOW_VERIFICATION.md`。真实收件箱投递、浏览器注册与密码找回全流程、所有历史详情页面及正式部署后验收仍未覆盖。

### 2026-09-09 并发采集验证补充（不发布）

- 用户暂不方便切换 App 凭据，本轮不合并、不部署、不停止线上训练。
- 新增隔离 PostgreSQL 并发场景：4 位模拟患者、8 个设备、320 个唯一样本及 320 个重复补传，验证回执、去重、计数、跨患者拒绝和冲突 ID；接入 CI。执行结果以本轮 GitHub 检查为准。
- 新增 `docs/CAPACITY_VERIFICATION.md`，记录测试边界和后续全链路容量验收方法。仅限回环地址、专用测试库；不向正式服务器上传模拟数据。
- 此场景不覆盖手机、HTTP 中间件或网页渲染，不能据此声称延迟已低于 2 秒或确定批量患者容量。
- 本日 CI 发现新增依赖安全告警，旧提交昨天通过不代表今日仍通过。已将 Next.js 与配套 ESLint 配置升至 15.5.25、js-yaml override 升至 4.3.2、sharp override 升至 0.35.4；锁文件重新审计为 0 漏洞，完整回归继续核验。生产尚未获得这些补丁。
- 最终代码 `4c3f284` 完整 CI `34321806568` 通过：安全扫描、99 项普通回归、11 项原有真实数据库测试、新增并发采集测试、运维测试、构建和隔离恢复均成功。本地类型检查通过。
- 并发基线：320 唯一样本 + 320 重传，4 位患者，总耗时 2942ms，批次 P50 1027ms、P95 2078ms；不作为全链路延迟或容量承诺。修复通知测试在发送查询尚未结束时关闭连接造成的偶发未处理异常。

### 2026-09-09 发布前生产备份恢复核验

- PR #50 最新代码 `d9b124c` 的完整 GitHub CI 已通过：运行 `34245695018`。分支尚未合并或部署。
- 密钥 SSH 已连通。服务器 `/opt/tka-rehab/.deployed-commit` 仍为 `2237311ad05a16efa4aa5279996227b1b0777c56`，应用与数据库健康，磁盘可用约 14 GB。
- 使用当前分支备份脚本创建真实生产备份：`/opt/tka-rehab/backups/release-20260909/tka-rehab-20260909T064138Z-wW5jQG2z.dump`，1,374,947 字节，私有权限。脚本放于独立运维目录，未覆盖线上应用代码。
- 使用当前分支恢复脚本将备份恢复至禁用网络、使用临时内存存储的独立 PostgreSQL 容器，应用表查询通过；演练容器已删除，复查无残留。未输出患者数据，也未把备份下载或上传 GitHub。
- 演练后公网 `/api/health/ready` 返回 ready，现有生产容器保持运行，未执行正式数据库迁移。
- 边界：本次证明同机生产备份可恢复，不代表异地恢复、新版本迁移或物理传感器验收通过。异地存储与告警接收位置仍未提供。
- 下一步：协调 App 凭据切换窗口，演练新版本迁移后再发布 PR #50；新版本拒绝旧共享 Token，需从家属设备页或主管护士档案页签发专属凭据并填入 App。不要未告知用户就切断现有上传。


### 2026-09-08 软件交付加固收尾（核心验证通过，尚未部署）

2026-09-08 收尾：检查点 `e59413b` 的 GitHub CI `34203633203` 全通过：99 项常规测试、11 项真实 PostgreSQL 测试、13 项运维/静态检查、17 项模拟脚本场景，以及迁移、构建、依赖审计。合成数据库成功恢复至隔离容器并查询应用表。补修护士手机页底部导航遮挡凭据按钮，生产构建通过；家属/护士页面在 390/1280 宽度均正常点击签发、显示结果且无横向溢出（UI 响应模拟，授权另有真实数据库验证）。用户尚无异地对象存储/告警接收配置，未购买服务，真实异地备份和通知仍待配置。完整任务矩阵见 `docs/TASK_CLOSEOUT_20260908.md`。

- 新增数据库存储哈希的患者/设备范围采集凭据，90 天过期、显式撤销、家属设备页及主管护士档案页签发入口；旧共享 Token 不再接受。新增迁移 `20260906000000_scoped_gateway_credentials`，上线前须为 App 更换凭据。
- 设备增加患者所有权，禁止跨患者抢绑/覆盖资料；上传、心跳、会话结束和批量路径均检查授权范围；设备输出不再通过对象展开暴露内部字段。归属解除同步撤销相关凭据。
- 多账号业务补充真实路由回归及可运行的独立 PostgreSQL 并发测试，修补移交/解除关联竞态；CI 新增隔离 PostgreSQL 迁移和测试门禁。
- 新增 restic 异地加密备份选项、隔离恢复演练、健康/备份年龄监控与 systemd 单元。CI 合成数据库隔离恢复通过；真实异地存储、告警投递及生产恢复尚未验收，不能宣称容灾达标。
- 旧 Supabase 匿名开放 SQL 已替换为后端专用访问基线，附清理迁移及核查脚本；并未连接或修改实际 Supabase 数据库。
- 增加运维账号/设备生命周期脚本，实时事件通过 PostgreSQL 通知跨进程分发；SSE 每 60 秒重连重验授权。真实吞吐容量未测定，多实例共享限流仍需扩容前处理。
- 用户明确暂缓实物长时间同步及算法参考角度验收。这两项、工厂出厂检测不能由自动化软件测试代替。后续代理先查看本节与 `docs/SOFTWARE_HARDENING_20260906.md`，不要误把未部署分支当作线上成果。

### 产品与业务闭环

- 已统一家属端与护士端患者病历数据源：患者姓名、出生日期、性别、民族、籍贯、国籍、婚姻状况、职业、血型、联系方式、紧急联系人、过敏状态与详情、既往史、手术史、家族史、用药史和本次 TKA 信息统一写入 `Patient`；账号联系人 `Profile` 不再冒充患者资料。双方通过同一 API 保存和读取，病历修改写入审计。
- 已建立主管护士强归属：一名患者最多一名主管护士，一名护士可管理多名患者；护士生成 48 小时一次性归属码，患者建档后填写，护士工作台及患者相关接口只返回本人负责患者。换护士必须由原护士先解除再使用新码，已绑定患者不能由家属单方面解除。详细契约见 `docs/PATIENT_RECORD_AND_NURSE_OWNERSHIP.md`。
...
- 2026-09-08：本地加固分支已同步 GitHub PR #50，仍未合并 main、未部署服务器；不要将分支验证等同线上验收。保留本地 `.claude/`、`artifacts/`、`debug.log`，未纳入提交。上一代理核心软件工作已完成并验证，但硬件/算法/生产业务/真实容灾/容量和工厂验收尚未闭环。
- 2026-09-05 同步核查：本地与 origin/main 起点均为 d625c65；另有未合并 UI 草稿 PR #49。本轮修复档案侧栏遮挡、无患者无限加载和手术早于出生日期校验，并修补间接依赖。详见 `docs/DELIVERY_READINESS_20260905.md`；本轮修改尚未部署服务器。

- 统一患者病历与主管护士归属已通过 PR #47 合并并部署生产：家属和主管护士共用同一份完整病历；一名患者最多一名主管护士，护士只读取本人负责患者。现有单护士部署的 1 名患者已正确回填主管护士，迁移后的家属关联、护士患者列表和护士完整档案接口均通过生产登录态验证。
- 本轮门禁：80 项 Web/网关/API 测试、Prisma schema、TypeScript、ESLint、49 路由生产构建和 GitHub Build 通过；生产备份副本已在临时 PostgreSQL 数据库完整恢复并成功执行第 15 项迁移，复核完成后临时库已删除。正式数据库当前无待执行迁移。

- 公开注册后的患者建档与授权关联已通过 PR #45 合并并部署生产：未关联家属可明确确认后创建自己的患者档案，或使用护士生成的 48 小时一次性授权码关联既有患者；家属和护士均可撤销访问，建档、邀请、接受与撤销均写入审计记录。授权码只在生成时显示明文，数据库仅保存 HMAC 摘要；生产使用独立 `PATIENT_INVITE_SECRET`，既有账号关联已回填迁移审计。
...
- 默认分支与当前交付基线：`main`；生产部署提交为 `2237311ad05a16efa4aa5279996227b1b0777c56`，包含 PR #47 的统一患者病历、主管护士强归属、一次性归属码和患者范围权限过滤。
...
- 正式服务器：`103.242.13.17` 已部署 Docker Compose 生产栈（PostgreSQL、Next.js、Caddy），当前源码为 GitHub `main` 合并提交 `2237311ad05a16efa4aa5279996227b1b0777c56`（PR #47，统一患者病历与主管护士强归属），`.deployed-commit` 已同步；本地签名角色会话、业务 API 保护、网关 Bearer Token、日志轮转、每日备份、防火墙与仅密钥 SSH 均已验证。`www.dorianaistudio.cloud` 已解析至正式服务器并取得有效 HTTPS 证书。
...
- 2026-08-24 患者关联闭环已部署：发布前备份 `/opt/tka-rehab/backups/tka-rehab-20260824T141550Z.dump`（1,365,888 字节，`pg_restore -l` 124 条），旧源码快照为 `/root/pre-deploy-a075a68-20260824T141615Z.tar.gz`；生产共 14 项迁移且无待执行项，三容器健康，家属/护士新权限读取、未授权拒绝与公网 HTTPS 均已验收。
- 2026-08-25 统一病历与主管护士归属已部署：发布前备份 `/opt/tka-rehab/backups/tka-rehab-20260824T161713Z.dump`（1,372,857 字节）且 `pg_restore -l` 可读，旧源码快照为 `/root/pre-deploy-2237311-20260824T162137Z.tar.gz`；备份副本迁移演练和正式第 15 项迁移均成功。三容器健康，现有患者归属 1/1，生产验收脚本、家属关联读取、护士患者范围、护士完整病历、公网 ready/login 和裸域跳转全部通过。

## 下一步任务

按优先级从上到下执行：

优先按 `docs/TASK_CLOSEOUT_20260908.md` 的发布顺序执行。核心软件回归已通过；PR #50 尚未上线。硬件长测及算法对照按用户要求暂缓，未完成前不扩大正式医疗交付。

1. 核对生产版本、备份与回滚条件，受控发布 PR #50；先迁移数据库，再签发患者/设备专属凭据并更新 App。旧共享 Token 会被拒绝，不得恢复全局授权绕过。
2. 用专用测试账号完成线上邮箱注册、改密/重置、护士归属/移交、资料同步、历史查询、护理指导和预警处理验收；真实数据库核心隔离/并发测试已通过，不能替代整条线上流程。
3. 配置真实异地加密存储及告警接收位置，验证投递与生产恢复；测定并发容量，扩容多实例前实现共享限流。完善设备批次、升级回滚及工厂质检证据。
4. 具备现场条件后做双传感器长时弱网验收：最新帧延迟 P95 不超过 2 秒，队列收敛，补传不重复，断开不伪装实时；用参考角度量化 ROM 误差、校准重复性及正常动作误报率。
...
2026-09-08：复核上一代理提交与 CI 全通过证据；修复护士手机页凭据按钮被底栏遮挡，四种角色/尺寸组合浏览器测试及生产构建通过。补充逐项完成边界与发布顺序，同步 PR #50；未合并、未部署，未宣称实物/临床/异地容灾验收完成。

2026-09-06：已实现范围凭据/设备所有权及采集接口鉴权、多账号竞态修补、后端专用 Supabase 基线、备份/监控/恢复工具及生命周期 CLI。当前 13 项新增权限/路由测试通过、17 项模拟运维场景通过、12 项运维/SQL静态检查通过（Windows 跳过 1 项权限检查）；类型检查通过。完整构建及 CI 真数据库测试进行中，未部署。下一步核验 CI、更新最终结果；异地凭据和现场硬件验收仍需外部条件。

| 日期 | 已完成事项 | 当前状态 | 下一步任务 | 验证 |
| --- | --- | --- | --- | --- |
| 2026-09-05 | 核对本地/main/UI草稿；修复档案侧栏遮挡、空档案加载与日期校验；修补间接依赖并编制批量交付门槛 | 本地修复，未部署；保留其他代理草稿与本地辅助目录 | 完成设备独立授权和实物长测，按交付门槛推进 | 81项测试及生产构建通过；390/1280/1920布局无横向溢出，桌面内容起点与侧栏右边界均256px |
| 2026-08-25 | 合并 PR #47，备份生产数据库，在备份副本演练统一病历与主管护士归属迁移后部署 `2237311`；核验家属关联、护士患者范围和完整病历读取 | 正式数据库 15 项迁移全部生效，现有患者归属 1/1，三容器健康，公网域名正常；测试副本已删除，备份与旧源码快照保留 | 使用两个测试护士账号完成“绑定→拒绝抢占→原护士释放→新护士接管”的非破坏性生产验收 | 备份 1,372,857 字节且 `pg_restore -l` 可读；迁移副本恢复/迁移/字段与回填复核通过；`verify-production.mjs` 通过；家属关联 200、护士列表 200/1 人、完整病历 200；ready/login 200、裸域 301 |
| 2026-08-24 | 统一患者病历数据源；补齐医院常用人口学、联系人、过敏史与健康史字段；建立一患者一主管护士的强归属码、移交流程、后端患者范围过滤和变更审计；增加家属/护士共享病历界面与自动刷新 | 代码与本地门禁通过，尚未合并或部署；迁移会把既有患者回填给最早护士，生产执行前需数据库备份与归属复核 | 推送 PR 并通过 GitHub CI；在隔离 PostgreSQL 演练迁移后备份生产、部署并以两个护士账号完成越权验收 | 80/80 Web/网关/API 测试；TypeScript、ESLint、Prisma validate、49 路由构建；1440/390 双视口 0 横向溢出，浏览器保存同步流程通过；本机 Docker 未运行，迁移演练待服务器阶段完成 |
| 2026-08-24 | 完成并部署公开注册后的患者闭环：家属自助建档、护士 48 小时一次性授权码、接受/撤销权限、角色边界与完整访问审计；补充资料页双角色管理界面和注册后建档引导；修复 CI 新发现的 3 项间接依赖安全公告 | PR #45 已合并，生产运行 `a075a68`；14 项迁移生效，三容器健康，独立邀请密钥已配置 | 用新注册测试账号完成创建、邀请、接受、双方撤销及多账号越权验收，再继续双传感器长时弱网验证 | 75/75 Web/网关/API 测试；TypeScript、ESLint、Prisma validate、49 路由生产构建和 `npm audit` 0 漏洞；GitHub Build `32736616735` 通过；生产备份 1,365,888 字节/124 条目录；`verify-production.mjs`、家属/护士 `/api/patient-access`、未授权 401、ready/login 200、裸域 301 全部通过 |
...
### 2026-08-24 生产部署：患者建档、授权与审计闭环（a075a68）

PR #45 已合并到 GitHub `main`，合并提交
`a075a68ea32207af03ae079cb318a2e9444ed01b` 已部署到正式服务器。发布前创建数据库
备份 `/opt/tka-rehab/backups/tka-rehab-20260824T141550Z.dump`（1,365,888 字节），
并通过 `pg_restore -l` 校验得到 124 条可恢复目录记录；旧源码快照保存为
`/root/pre-deploy-a075a68-20260824T141615Z.tar.gz`。发布归档在本机与服务器的
SHA256 均为 `d36d95a10bfd35bb1bf16041be5d04391587c930bfdd491199fd017dac7cf7d9`。

服务器私有 `.env.production` 已生成独立 `PATIENT_INVITE_SECRET`，其值未输出、未下载、
未提交仓库。迁移 `20260824143000_add_patient_onboarding` 成功执行，数据库共 14 项迁移
且无待执行项；邀请表初始为空，既有正式家属关联回填 1 条 `MIGRATED` 审计记录，未创建
模拟患者或传感器数据。

PostgreSQL、Next.js 与 Caddy 三容器健康，`.deployed-commit` 已写入完整合并提交。
容器内 `deploy/verify-production.mjs` 通过健康、角色隔离、受保护数据、网关授权、实时页
和传感器流检查；生产默认家属和护士会话分别读取 `/api/patient-access` 成功，未登录访问
返回 401。公网 `ready` 与登录页返回 200，裸域保留路径重定向到 `www`，每日备份定时器
保持 active。
