...
- 正式服务器：`103.242.13.17` 已部署 Docker Compose 生产栈（PostgreSQL、Next.js、Caddy），当前源码为 GitHub `main` 合并提交 `7ee5e14`（含全站视觉改版、登录后应用骨架、Android v0.4.4 视觉统一、安全加固及桌面侧栏固定定位修复），`.deployed-commit` 已同步；本地签名角色会话、业务 API 保护、网关 Bearer Token、日志轮转、每日备份、防火墙与仅密钥 SSH 均已验证。`www.dorianaistudio.cloud` 已解析至正式服务器并取得有效 HTTPS 证书。
