...
test("keeps an offline sample until a retry receives a successful upload response", async () => {
  ...
  try {
    ...
    assert.deepEqual(uploads, ["gateway-sample-replay-001", "gateway-sample-replay-001"]);
  } finally {
    await gateway.stop();
  }
});
