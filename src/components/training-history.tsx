...
export function TrainingHistory({ role }: { role: "family" | "nurse" }) {
  ...
  return (
    <main className="rehab-grid min-h-screen px-4 pb-28 pt-5 text-[#12304a] md:px-10">
      <section className="mx-auto max-w-6xl space-y-5">
        <header className="flex flex-col gap-4 border-b border-slate-200 bg-white p-5 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-black">近 15 天训练记录</h1>
        ...
        {!loading && sessions.length === 0 ? <div className="bg-white p-10 text-center"><Activity className="mx-auto size-9 text-slate-400" /><p className="mt-3 font-bold">还没有已结束的训练</p></div> : null}
            ...
            return (
              <article key={session.id} className="border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div>
                    <div className={`inline-flex items-center gap-2 px-3 py-1 text-sm font-black ${presentation.className}`}><Icon className="size-4" />{presentation.text}</div>
                    <h2 className="mt-3 text-xl font-black">{session.patientName} · {new Date(session.startedAt).toLocaleString("zh-CN")}</h2>
                ...
                {!professional ? (
                  <div className="mt-5 grid gap-3 md:grid-cols-2">
                    <div className="bg-slate-50 p-4">
                      <p className="text-sm font-bold text-slate-500">系统怎么看</p>
                    ...
                    <div className="bg-slate-50 p-4">
                      <p className="text-sm font-bold text-slate-500">接下来怎么做</p>
                  ...
                  <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                    ...
                    ].map(([label, value, suffix]) => <div key={String(label)} className="border-l-2 border-emerald-600 bg-slate-50 p-3"><p className="text-xs font-bold text-slate-500">{label}</p><p className="mt-2 text-xl font-black">{typeof value === "number" ? value : "--"}{suffix}</p></div>)}
                    {(metrics?.warnings ?? []).map((warning) => <div key={warning.code} className="sm:col-span-2 lg:col-span-5 border-l-4 border-amber-500 bg-amber-50 p-3"><p className="font-bold">{warning.title}</p><p className="mt-1 text-sm">{warning.evidence}</p><p className="mt-1 text-sm font-semibold">建议：{warning.action}</p></div>)}
                    {session.analysis ? <div className="sm:col-span-2 lg:col-span-5 border-l-2 border-slate-400 bg-slate-50 p-3"><p className="font-bold">智能解读 · {session.analysis.status ?? "待标注"}</p><p className="mt-1 text-sm">{session.analysis.report}</p><p className="mt-1 text-sm">建议：{session.analysis.recommendation}</p></div> : null}
                  </div>
                )}
              </article>
            );
