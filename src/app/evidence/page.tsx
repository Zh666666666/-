  ...
  return (
    <main className="rehab-grid min-h-screen px-4 pb-40 pt-4 text-slate-950 md:px-10 md:pb-10 md:pt-6">
      <section className="mx-auto max-w-7xl space-y-5">
        <header className="rounded-xl border border-[var(--hairline)] bg-[var(--surface)] p-5 shadow-e2 md:p-6">
          ...
          <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h1 className="text-3xl font-medium text-[#12211c] md:text-4xl">真实硬件任务回放与事件处置</h1>
          ...
          <>
            <section className="space-y-4" aria-labelledby="evidence-summary-title">
              <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                <div>
                  <p className="text-sm font-medium text-emerald-700">任务 {evidence.session.id}</p>
                  <h2 id="evidence-summary-title" className="mt-1 text-2xl font-medium text-[#12211c]">采集结果</h2>
              ...
              <CardContent className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Line type="monotone" dataKey="roll" name="Roll°" stroke="#3c6552" dot={false} strokeWidth={2} />
                    <Line type="monotone" dataKey="pitch" name="Pitch°" stroke="#2f6076" dot={false} strokeWidth={2} />
                    <Line type="monotone" dataKey="yaw" name="Yaw°" stroke="#8f6427" dot={false} strokeWidth={2} />
                  </LineChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
            ...
            <section className="space-y-3" aria-labelledby="event-review-title">
              <div>
                <p className="text-sm font-medium text-emerald-700">处理闭环</p>
                <h2 id="event-review-title" className="mt-1 text-2xl font-medium text-[#12211c]">异常事件确认</h2>
              </div>
                  ...
                  <Card key={event.id} className="border-[var(--hairline)] bg-white shadow-e2">
                    <CardContent className="grid gap-4 p-5 lg:grid-cols-[1fr_0.8fr]">
                      <div>
                        ...
                        <h3 className="mt-3 text-lg font-medium text-[#12211c]">{event.title}</h3>
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xl font-medium text-[#12211c]">{loopComplete ? "本次任务闭环完成" : "仍有事件待处理"}</h2>
                  </div>
