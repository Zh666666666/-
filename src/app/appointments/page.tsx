...
import { CalendarClock, CheckCircle2, ClipboardList, Radio, Settings2, XCircle } from "lucide-react";
...
function AppointmentManageDialog({ appointment, onSubmit }: { appointment: AppointmentItem; onSubmit: (id: string, status: AppointmentStatus, scheduledTime: string | null, responseNote: string) => Promise<void> }) {
  ...
  const [responseNote, setResponseNote] = useState("已安排骨科康复护士上门，请保持电话畅通，并提前准备近期训练记录。");
...
export default function AppointmentsPage() {
  ...
  const [description, setDescription] = useState("希望护士上门评估膝关节肿胀、屈膝角度和步态训练情况。");
            ...
            <Card key={appointment.id} className="bg-white/90">
              <CardContent className="grid gap-4 p-5 md:grid-cols-[1fr_auto] md:items-center">
                <div>
                  ...
                  {appointment.responseNote ? <p className="mt-2 rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-600">护士回复：{appointment.responseNote}</p> : null}
